 ...
 *            pos : String  
	          ...
	          in : BufferedReader // 접속자 요청	    
	           out : OutputStream    
 *      ------------------------------------
 *            + Client(Sockrt s) : 쓰레드 연결 => 클라이언트마다 따로 통신
 *            + run : void : 쓰레드 통신(클라이언트와 연동)
 *                 => case 문장 이용
 *            + messageTo(String msg) : void : 반복 제거 => 한명에 데이터 전송
 *            + messageAll(String msg) : void : 접속자 모두에세 전송
 *      -------------------------------------------------------------------
 *            ------
	    	...
	    	try {
	    		...
	    		while(true) {
	    			...
	    			case Function.CHATEND:{
	    				messageAll(Function.WAITCHAT + "|[알림 →]" + name + "님이 퇴장했습니다|red");
