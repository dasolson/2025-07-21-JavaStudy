 ...
 *     변수                        연산자 / 제어문            화면 출력
 *      |                              |                    |
 *   데이터 저장방법(메모리)          변수에 대한 연산처리        가공된 데이터 출력
 *    => 한개 저장 : 변수           메소드
 *    ---------------- 기본형     데이터 가공
 *    (이미지정된 메모리 크기)
 *    => 같은 데이터 여러개 : 배열
 ...
 *            ---- 오버로딩
 *       --- 오버라이딩
 *    4. 추상화 : 공통모듈 => 공통적인 => 소스 간결
 *        => 추상클래스 / 인터페이스(추상 클래스의 확장)
 *    ------------------------------------------- 객체지향의 4대 요소
 *    
 *    객체 지향 제작
 *    ----------
 *    클래스 : 데이터 여러개 묶어서 메모리에 한번에 저장하기 위한 설계
 *           ---------- 관련된 데이터를 묶는다
 *           ex) 맛집, 게시판, 영화, 음악...
 *    
 *    클래스의 구성요소
 ...
 *          - MethodArea에 저장
 *          - 공통으로 사용되는 변수가 있는 경우
 *            --------- 데이터 베이스(오라클, MySQL) => 공유변수
 *                      ex) 로고
 *        지역 변수        
 *          - 주로 메소드에서 사용
 *          - 저장되는 시점 : 메소드 호출
 *          - 종료 시점 : 메소드가 종료시에 자동 회수
 *          - 스텍 공간에 저장
 *          -유사한 변수 : 매개변수
 *      ---------------------------------------------------
 *      1. 선언위치
 ...
 *                       ---
 *                       ex) 연산처리/제어문
 *                           파일 읽기/ 데이터베이스 연결 / 서버연결
 *                           int a=10;  // 기능 => 명시적 초기화
 *                           int b;
 *                           b=20; // 구현 => 오류    
 *                           
 *                           시작과 동시에 선언
 *                            ex) 자동 로그인 / 아이디 읽기 / 파일 읽기 
 *                   }       
 *        *** 정적변수  
 *              - 클래스명.변수명  => 정적변수와 구분 static
 *                 class ClassName {
 *                     static int a; => 0값 초기화
 ...
 *                     => 인스턴스 호출시에는 반드시 클래스 메모리 할당을 해야한다
 *        *** 지역변수
 *              - 메소드 안에서만 사용 가능 : 변수명만
 *        *** 지역변수가 우선 순위 제일 상위
 *           -------------------------
