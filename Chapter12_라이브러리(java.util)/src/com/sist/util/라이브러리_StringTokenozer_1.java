 ...
 *     util : 유용한 클래스의 집합
 *     
 *   *** 1) StringTokenozer : 문자 분해
 *       
 *   *** 2) Collection
 *       
 *           = List 형 => 순서를 가지고 있다(인덱스) , 데이터 중복 허용
 *                  => 데이터베이스(비동기), 네트워크의 접속자 저장(동기)
 *               ArrayList, Vector, LinkedList, Queue
 *               ---------  -------
 *                 비동기       동기 
 *               
 *           = Set 형 => 순서가 없다, 데이터 중복을 허용하지 않는다
 *               HachSet, TreeSet
 *               
 *           = Map 형 => 순서가 없다, (키, 값)쌍으로 되어있다
 *               HashMap, Hashtable
 *               -------  ---------
 *                비동기      동기 
 *          
 ...
 *        
 *        StringTokenozer : 구분자별로 문자열을 나눠주는 역할
 *              1) 생성자 => 구분
 ...
 */
import java.util.*;
public class 라이브러리_StringTokenozer_1 {
