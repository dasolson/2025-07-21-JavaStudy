...
/*
 *      인터페이스 : default 가 없는 경우
 *          ex) 버튼 클릭
 *              마우스 클릭
 *         ** 이벤트는 인터페이스로 되어있다
 *         
 *      1. 버튼 클릭 / 메뉴 클릭 / 텍스트 필드 엔터
 *         ------------------------------- ActionListener
 *            => actionPerformed()
 *      2. 테이블 / 트리 / 레벨 / 이미지
 *         ----------------------- MouseListener
 *            => mouseClicked()  
 *            => mousePressed()  ex) 누른상태로 이동
 *            => mouseReleased() ex) 마우스를 놨을때
 *            => mouseEntered()  ex) 툴바에 마우스 커서 올렸을때
 *            => mouseExited()   
 *      3. Keyboard
 *         -------- KeyListener
 *            => KeyTyped()    ex) 키 여러개 눌렀을때
 *            => KeyPressed()
 *            => KeyReleased()            
 *      4. combox, list
 *         --------------------- ItemListener  
...
public class 예외처리_5 extends JFrame implements ActionListener{
    ...
    int s,b;
    // 초기화
    // 생성자 => instance / 초기화블록 => static {} : static
    public 예외처리_5() {
    	/*
    	 *  접근지정어
    	...
    	tf=new JTextField(10); // NumberFormatException 문자열
    	tf.setEnabled(false);
    	// 웹 / 윈도우 => Only String
    	// --------- 크롤링(String) 
    	ta=new JTextArea();
...
//				JOptionPane.showMessageDialog(this, "중복된 수는 사용 할 수 없습니다");
				tf.setText("");
				tf.requestFocus();
				setMessage("중복된 수는 사용 할 수 없습니다");
				return;
			}
