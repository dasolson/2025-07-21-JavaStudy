package com.sist.util;

import java.util.ArrayList;
import java.util.LinkedList;
import java.util.List;
import java.util.Vector;

/*
 *     Collection
 *      1. 데이터를 묶어서 쉽게 관리 할 수 있게 만든 클래스의 집합
 *         ------------------- 자료구조(스택, 큐, 데큐)
 *         
 *      2. Collection Framework
 *                    --------- 표준화 => 동일한 형태의 소스 코딩
 *                              Spring / MyBatis / JQuery / Vue / React
 *                              일정한 틀 => 코딩 분석이 쉽다
 *      3. 배열의 단점을 보완
 *         -------- 고정적 => 가변형으로 바꿈
 *      4. 종류
 *                Collection (interface)
 *                ----------------------
 *                          |
 *            ---------------------------- => interface
 *            |            |             |
 *          List          Set           Map  
 *            |            |             |
 *            |        ---------       ---------------
 *            |        |       |       |             |
 *            |     HashSet  TreeSet Hashtable   HashMap
 *            |
 *      -------------------- 구현된 클래스
 *      |         |        |
 *   ArrayList Vector  LinkedList
 *                         |
 *                       Queue
 *      
 *       List
 *         1) 순서를 가지고 있다(인덱스 번호) => 0
 *         2) 데이터 중복 저장 허용
 *         3) CRUD를 주로 사용
 *             (추가, 읽기, 수정, 삭제)  
 *         4) 비동기적 => 속도가 빠르다 => 데이터베이스 사용
 *         5) 주요 기능 
 *              - 데이터 추가    : add(Object)
 *              - 데이터 읽기    : get(int index)
 *              - 데이터 수정    : set(int index, Object)
 *              - 데이터 삭제    : remove(int index)
 *              - 데이터 갯수    : size()
 *              - 전체 삭제     : clear()
 *              - 데이터 존재여부 : isEmpty() 
 *         6) 구현된 클래스
 *              - ArrayList  : 가장 많이 사용되는 클래스
 *                       => 데이터베이스(비동기적)
 *              - Vector     : 
 *                       => 네트워크(동기적) 
 *              - LinkedList : 양방향 => 저장되는 내용이 많아 메모리가 커진다              
 *         7) import 추가 => java.util  
 *    ----------------------------------------------------------------
 *       Set
 *         1) 순서가 없다(인덱스가 없다)
 *         2) 중복된 데이터를 허용하지 않는다
 *             ex) add(1) add(1) add(2) => add(1) add(2) 
 *         3) 주로 사용 => List 에 중복된 데이터 제거
 *         4) 구현된 클래스
 *              - HashSet
 *              - TreeSet
 *         5) 주요 기능 
 *              - 데이터 추가    : add(Object)
 *              - 데이터 읽기    : get()
 *              - 데이터 수정    : set(Object)
 *              - 데이터 삭제    : remove(Object)
 *              - 데이터 갯수    : size()
 *              - 전체 삭제     : clear()
 *              - 데이터 존재여부 : isEmpty()     
 *    ----------------------------------------------------------------                                                                           
 *        Map                  
 *                                     
 */
public class 라이브러리_컬렉션_1 {

	public static void main(String[] args) {
		// TODO Auto-generated method stub
//		ArrayList names=new ArrayList();
//        // 추가
//		names.add("홍길동"); // 0
//		names.add("박문수"); // 1
//		names.add("심청이"); // 2 -> 3
//		
//		System.out.println("현재 인원:" + names.size());
//		for(Object obj:names) {
//			System.out.println(obj);
//		}
//		
//		System.out.println();
//		names.add("이순신"); // 3 -> 4
//		names.add(2, "강감찬"); // 2
//		
//		System.out.println("현재 인원:" + names.size());
//		for(Object obj:names) {
//			System.out.println(obj);
//		}
//		
//		System.out.println();
//		
//		names.remove(0); // 인덱스 0번 삭제 => 나머지 인덱스 번호 당겨짐
//		System.out.println("현재 인원:" + names.size());
//		for(Object obj:names) {
//			System.out.println(obj);
//		}
//		
//		System.out.println();
//		names.set(2, "춘향이"); // 인덱스 번호 2번 내용 바꿈
//		System.out.println("현재 인원:" + names.size());
//		for(Object obj:names) {
//			System.out.println(obj);
//		}
//		
//		System.out.println();
//		names.clear(); // 전체 삭제
//		System.out.println("현재 인원:" + names.size());
//		for(Object obj:names) {
//			System.out.println(obj);
//		}
//		
//		if(names.isEmpty()) {
//			System.out.println("인원이 없다");
//		} 
		
		
		
//		Vector names=new Vector();
//        // 추가
//		names.add("홍길동"); // 0
//		names.add("박문수"); // 1
//		names.add("심청이"); // 2 -> 3
//		
//		System.out.println("현재 인원:" + names.size());
//		for(Object obj:names) {
//			System.out.println(obj);
//		}
//		
//		System.out.println();
//		names.add("이순신"); // 3 -> 4
//		names.add(2, "강감찬"); // 2
//		
//		System.out.println("현재 인원:" + names.size());
//		for(Object obj:names) {
//			System.out.println(obj);
//		}
//		
//		System.out.println();
//		
//		names.remove(0); // 인덱스 0번 삭제 => 나머지 인덱스 번호 당겨짐
//		System.out.println("현재 인원:" + names.size());
//		for(Object obj:names) {
//			System.out.println(obj);
//		}
//		
//		System.out.println();
//		names.set(2, "춘향이"); // 인덱스 번호 2번 내용 바꿈
//		System.out.println("현재 인원:" + names.size());
//		for(Object obj:names) {
//			System.out.println(obj);
//		}
//		
//		System.out.println();
//		names.clear(); // 전체 삭제
//		System.out.println("현재 인원:" + names.size());
//		for(Object obj:names) {
//			System.out.println(obj);
//		}
//		
//		if(names.isEmpty()) {
//			System.out.println("인원이 없다");
//		} 
		
		
		
//		LinkedList names=new LinkedList();
//        // 추가
//		names.add("홍길동"); // 0
//		names.add("박문수"); // 1
//		names.add("심청이"); // 2 -> 3
//		
//		System.out.println("현재 인원:" + names.size());
//		for(Object obj:names) {
//			System.out.println(obj);
//		}
//		
//		System.out.println();
//		names.add("이순신"); // 3 -> 4
//		names.add(2, "강감찬"); // 2
//		
//		System.out.println("현재 인원:" + names.size());
//		for(Object obj:names) {
//			System.out.println(obj);
//		}
//		
//		System.out.println();
//		
//		names.remove(0); // 인덱스 0번 삭제 => 나머지 인덱스 번호 당겨짐
//		System.out.println("현재 인원:" + names.size());
//		for(Object obj:names) {
//			System.out.println(obj);
//		}
//		
//		System.out.println();
//		names.set(2, "춘향이"); // 인덱스 번호 2번 내용 바꿈
//		System.out.println("현재 인원:" + names.size());
//		for(Object obj:names) {
//			System.out.println(obj);
//		}
//		
//		System.out.println();
//		names.clear(); // 전체 삭제
//		System.out.println("현재 인원:" + names.size());
//		for(Object obj:names) {
//			System.out.println(obj);
//		}
//		
//		if(names.isEmpty()) {
//			System.out.println("인원이 없다");
//		}  
		
		
		List names=new LinkedList();
        // 추가
		names.add("홍길동"); // 0
		names.add("박문수"); // 1
		names.add("심청이"); // 2 -> 3
		
		System.out.println("현재 인원:" + names.size());
		for(Object obj:names) {
			System.out.println(obj);
		}
		
		System.out.println();
		names.add("이순신"); // 3 -> 4
		names.add(2, "강감찬"); // 2
		
		System.out.println("현재 인원:" + names.size());
		for(Object obj:names) {
			System.out.println(obj);
		}
		
		System.out.println();
		
		names.remove(0); // 인덱스 0번 삭제 => 나머지 인덱스 번호 당겨짐
		System.out.println("현재 인원:" + names.size());
		for(Object obj:names) {
			System.out.println(obj);
		}
		
		System.out.println();
		names.set(2, "춘향이"); // 인덱스 번호 2번 내용 바꿈
		System.out.println("현재 인원:" + names.size());
		for(Object obj:names) {
			System.out.println(obj);
		}
		
		System.out.println();
		names.clear(); // 전체 삭제
		System.out.println("현재 인원:" + names.size());
		for(Object obj:names) {
			System.out.println(obj);
		}
		
		if(names.isEmpty()) {
			System.out.println("인원이 없다");
		}
	}

}

















