...
/*
 *     자바 소스
 *       package => 클래스가 잇는 폴더명 지정 => 한번만 사용 가능
 *       import  => 다른 패키지에 있는 클래스나 라이브러리 클래스 로드
 *                  여러번 사용이 가능
 *       class ClassName
 *       {
 *           사용할 변수 =>여러개 사용
 *             => 기본형 변수, 클래스, 배열, 클래스 배열
 *               ex) MusicVO vo;
 ...
 *                   => 배열, 형변환 가능
 *           생성자 => 여러개 사용이 가능 -> 오버로딩
 *                => 초기값을 다르게 저장한다면 매개변수가 있는 생성자 사용
 *           메소드 => 기능별로 메소드 제작
 *       }
 *       
 *       메소드
 ...
 *               - 재사용 가능 / 수정해서 사용 가능(오버라이딩)
 *           3) 메소드 구조
 *               - [접근지정어] 리턴형 메소드명(매개변수...)
 *                 {         ----
 *                     return 값;    => return과 리턴형이 동일하거나 리턴형이 크다(가급적 동일하게)
 *                 }          --
 *                 
