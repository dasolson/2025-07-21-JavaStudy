 ...
 *          ---------- 외부 라이브러리
 *          다운로드 / 업로드 / 웹파일
 *     => 파일 제어, 네트워크, 메모리 => 입출력 할 경우에 사용
 *     
 *     자바에서 지원하는 IO
 *         ASC (1byte) / Unicode(2byte)
 *                       ------------- 0~65535(각국의 언어 사용)
 *         ----------- 0~256(숫자, 알파벳)
 *          => 브라우저는 1byte(ASC) ==> Unicode 로 변경
 *                                    -------------
 *                                        UTF-8(웹 핵심)
 ...
 *             = **BufferedReader / BufferedWtiter 
 *           사용처 : 텍스트 파일 처리  
 *      3. 보조 스트림 => 속도 최적화(Buffered~)
 *           = 자바 프로그램 => JVM => 운영체제 => 하드디스크
 *           = 자바 프로그램 => JVM => 메모리(임시 저장 공간 : Buffer) ==> 최적화
 *           = BufferedInputStream / BufferedOutputStream
 *           = BufferedReader / BufferedWtiter
 *           사용처 : 네트워크 / 웹
 *      4. 객체 기반 스트림 => 시리얼라이즈
...
public class 입출력_2 {
	...
	public static void main(String[] args) {
		...
		try {
			 ...
			 String s=len==0?file.length() +"Byte":len+"KB";
