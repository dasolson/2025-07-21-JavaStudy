 ...
 *                             --------------- 1) 데이터 보호 목적 
 *                                             2) 경계선 
 *                                             3) 메소드와 변수를 한개로 묶어서 외부 접근 방지(getter/setter) 
 *                                             4) 접근지정어 사용법 
 *                                             5) 웹에서 주로 캡슐화 사용
 *             - 상속 
 *                기존의 변수와 메소드 재사용 => 확장
 ...
 *                                          ------ {}
 *                     **반드시 초기값을 부여
 *                     저장위치 : 스택 => 관리를 메모리 자체에서 관리
 *                     생명주기 : {}안에서만 사용 가능 => 블록 벗어나면 자동회수  
 *                      ex) class A
 *                          {
 *                              void display()
 *                              {
 ...
 *                         : 데이터 묶음, 오라클에 저장된 데이터   
 *                  *** 초기화 여부
 *                        지역변수 : 필수, 직접대입, 연산결과 대입, 기본값은 없다
 *                        인스턴스변수 : 선택, 선언시 대입, 생성자에서 대입, 기본값이 있다
 *                        정적변수 : 선택, 선언시 대입, 초기화 블록, 기본값이 있다
 *                  *** 초기화 방법
 *                        1) 선언과 동시에 초기화 : 명시적 초기화 =단점=> 고정(한가지 값)
 *                        2) 생성자를 통해서 초기화 => 여러가지 값을 지정 할 수 있다
 *                            => 사용자로부터 값을 받아서 저장이 가능
 *                            => 매개변수
 *                        3) 초기화 블록 =단점=> 고정(한가지 값)
 *                             --------------------
 *                              인스턴스 => {}
 *                              정적병수 => static {} 
 *                             -------------------- 파일 읽기 / 데이터베이스 / 쿠키 / 세션
 ...
 *                 정적변수 : 자동 저장 => static {}   
 *             *** 변수는 초기값이 있어야 메모리에 저장이 가능 => 프로그램에서 사용 가능
 *      ---------------------------------------------------------------------       
 *        생성자
 *          - 객체가 생성될때 호출되는 메소드 형태의 코드 블록
 *          - 역할 : 인스턴스 변수에 대한 초기값 대입 (초기화 함수)               
 *                : 시작과 동시에 처리해야 되는 기능 설정          
 *                  ex) 자동 로그인 / 아이디 저장을 전송  
 *                      쿠키를 읽어온다(장바구니) / 최근 방문
 *                      화면 UI => 레이아웃
 *          - 특징 : 클래스명과 동일
 *                : 리턴형잉 없다 => void도 리턴형
 *                : 오버로딩 지원 => 같은 이름의 메소드로 새로운 기능 제작
 *                                변수에 대한 초기값을 변경 => 매개변수
 *             *** 모든 클래스는 생성자가 한개이상 반드시 필요하다
 *                  => 생성자가 없는 경우에는 컴파일러가 자동으로 기본생성자 추가
 *                  => 기본 생성자 : 매개변수가 없는 생성자
 *          - 생성자의 종류 : 기본생성자(디폴트 생성자) => 자동 추가가 될 수 있다
 *                          => 시작돠 동시에 처리해야 되는 기능 설정
 *                       : 매개변수가 있는 생성자 
 *                          => 객체마다 다른 데이터를 저장하고 싶은 경우
 *          - 변수 : 기본형 => int a, double d...
 *                  --------------------------- 기본형 변수(실제값) 
 *                : 배열  => 같은 데이터형  
 *                : 클래스 => 다른 관련된 데이터형                
 *                  --------------------------- 참조형 변수(주소) 
 *          - this() 
 *            ------ 클래스 자신의 생성자 호출
 *                   호출되는 위치는 반드시 생성자 안에서 호출
 *                   생성자의 첫 줄에서만 사용할 수 있다                   
 *          - super()
 *            ------ 상속 내린 클래스의 생성자 호출
 *                   호출되는 위치는 반드시 생성자 안에서 호출
 *                   생성자의 첫 줄에서만 사용할 수 있다                                                         
